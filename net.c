  ...
  if (strstr(request, "POST /set") != NULL) {
    ...
    if (parse_post_data(request, &hour, &minute, &second)) {
      if (hour >= 0 && hour < 24 && minute >= 0 && minute < 60 && second >= 0 && second < 60) {
        ...
        char buf[256];
        int len = snprintf(buf, sizeof(buf), response, hour, minute, second);
    ...
    const char *response =
        ...
        "<script>var d=new Date();document.getElementById('h').value=d.getHours();document.getElementById('m').value=d.getMinutes();document.getElementById('s').value=d.getSeconds();</script>"
        "</body></html>";
